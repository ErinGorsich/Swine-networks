###############################################
###############################################
# Calculate the number of farms per county in ICVI & Nass datasets
# Run linear regression to compare
###############################################
###############################################
 # Outline
# 1) Read in and groom ICVI data, packages
...
data.cvi$MOVE <-1
data <-data.cvi[ , c("STATE", "SAMPLE_YEAR2", "PURPOSE", 
	"NUM_SWINE", "NUM_BOAR", "NUM_BARROW", "NUM_GILT", 
  	"NUM_SOW", "NUM_AGE_0.2_MONTHS", "NUM_AGE_2.6_MONTHS", 
  	"NUM_AGE_6._MONTHS", "NUM_MALE", "NUM_FEMALE",
  	"D_STATE", "D_FIPS_X", "D_FIPS_Y",  "O_FIPS_X", 
  	"O_FIPS_Y", "O_STATE", "D_FIPS", "O_FIPS", 
  	"O_ST_FIPS", "D_ST_FIPS", "MOVE" )]
...
calculate_number_farms=function(data){
	# function reads in the CVI data and returns a data.frame 
    ...
    node.stats <- data.frame(matrix(0, 
    	...
    	ncol = 5,
        dimnames = list(NULL, c("NodeID", "StateID", "NumFarms",
        "NumOFarms","NumDFarms"))))
	...
	temp.o.loc.data <- unique(cbind(as.factor(data$O_FIPS_X), 
		as.factor(data$O_FIPS_Y), data[,"O_FIPS"], data$MOVE))
		
	o.temp <- aggregate(as.numeric(temp.o.loc.data[,4]), list(temp.o.loc.data[,3]), FUN = sum)
	for (k in 1:length(o.temp$Group.1)) {
		node.stats$NumOFarms[as.numeric(node.stats$NodeID) == 
        	as.numeric(o.temp$Group.1[k])] = o.temp$x[k]
	}
   ...
   temp.d.loc.data <- unique(cbind(as.factor(data$D_FIPS_X), as.factor(data$D_FIPS_Y),
    ...
    for(k in 1:length(d.temp$Group.1)){
    	node.stats$NumDFarms[as.numeric(node.stats$NodeID) == 
    		as.numeric(d.temp$Group.1[k])]=d.temp$x[k]
    }
    return(node.stats)
}
...
num_cvi_farms<- calculate_number_farms(data)

# number of farms in abstract
sum(num_cvi_farms$NumFarms)

# subset node.stats to only include states with data
stateswant10<-c(6, 19, 27, 36, 37, 48, 55) #40,
stateswant11<-c(6, 19, 27, 36, 37, 48, 55) #, 31, 40
...
cvidata10 <- droplevels(cvidata10)
cvidata11 <- droplevels(cvidata11)


# plot data to check model assumptions
hist(cvidata10$NumFarms)  
...
###############################################
###############################################        
NASS <- read.csv("NASS/NASS2012.csv")         
dataframe <- NASS 
...
finaldata2010$NumCVIFarms <- cvidata10$NumFarms[match(finaldata2010$FIPS, 
	cvidata10$NodeID)]
finaldata2010 <- finaldata2010[!is.na(finaldata2010$NumCVIFarms),]
finaldata2011 <- newdataframe
finaldata2011$NumCVIFarms <- cvidata11$NumFarms[match(finaldata2011$FIPS,
	cvidata11$NodeID)]
...
points(finaldata$NumCVIFarms[finaldata$border_ind == 1] ~ 
	finaldata$newvalue[finaldata$border_ind == 1], pch = 19)
...
full.mod.pois <- glm(NumCVIFarms ~  state + newvalue + border_ind + year + 
	state*newvalue + state*border_ind + state*year +
	newvalue*border_ind + newvalue*year + border_ind*year, 
	data = finaldata[which(fitted(test.mod) < 60), ], family = poisson(link = "log"))     
...
QAIC(full.mod.pois, chat=chat) # 1557.41
QAIC(drop.p1, chat=chat) 
QAIC(drop.p2, chat=chat)
QAIC(drop.p3, chat=chat)
QAIC(drop.p4, chat=chat) # lowest, remove boarder*year 1550.7
...
QAIC(drop.p1, chat=chat) # 1549
QAIC(drop.p2, chat=chat)
QAIC(drop.p3, chat=chat)
QAIC(drop.p4, chat=chat) # 1547
QAIC(drop.p5, chat=chat)
...
drop.mod4<- glm(NumCVIFarms ~  state + newvalue + border_ind + year + #state*newvalue +
						  newvalue*border_ind + newvalue*year + border_ind*year, 
						  data = finaldata[which(fitted(test.mod) < 60), ], family = quasipoisson(link="log")) 
drop.p4 <- update(drop.mod4, family=poisson)

QAIC(drop.p1, chat=chat) # 1545  
QAIC(drop.p2, chat=chat) 
QAIC(drop.p3, chat=chat)
...
###############################################
###############################################
# run model with each year separately (not used)
###############################################
###############################################               
# additive model first
test.mod<-glm(NumCVIFarms~ state+newvalue+border_ind, data=finaldata2011, family=quasipoisson(link="log")); extractAIC(test.mod)     

# conduct backwards selection
library("MuMIn")
#library("bbmle")
full.mod<-glm(NumCVIFarms~ state*newvalue+state*border_ind+ newvalue*border_ind, data=finaldata2011, family=quasipoisson(link="log"))     
full.mod.pois<-glm(NumCVIFarms~ state*newvalue+state*border_ind+ newvalue*border_ind, data=finaldata2011, family=poisson(link="log"))     
...
drop.mod1<-glm(NumCVIFarms~ state*border_ind+ newvalue*border_ind, data=finaldata2011, family=quasipoisson(link="log"))     
drop.p1<-update(drop.mod1, family=poisson)
drop.mod2<-glm(NumCVIFarms~ state*newvalue+ newvalue*border_ind, data=finaldata2011, family=quasipoisson(link="log"))  
drop.p2<-update(drop.mod2, family=poisson)
drop.mod3<-glm(NumCVIFarms~ state*newvalue+state*border_ind, data=finaldata2011, family=quasipoisson(link="log"))     
drop.p3<-update(drop.mod3, family=poisson)
...
drop.mod2<-glm(NumCVIFarms~ state*newvalue+ newvalue*border_ind, data=finaldata2011, family=quasipoisson(link="log"))  
drop.p2<-update(drop.mod2, family=poisson)
drop.mod4<-glm(NumCVIFarms~ state+newvalue+border_ind+ newvalue*border_ind, data=finaldata2011, family=quasipoisson(link="log")) 
drop.p4<-update(drop.mod4, family=poisson)
drop.mod5<-glm(NumCVIFarms~ state*newvalue+ newvalue+border_ind, data=finaldata2011, family=quasipoisson(link="log"))  
drop.p5<-update(drop.mod5, family=poisson)

QAIC(drop.p2, chat=chat)
QAIC(drop.p4, chat=chat)
QAIC(drop.p5, chat=chat)  # selected qAIC= 954.7581

# repeat dropping of last interaction term
drop.mod5<-glm(NumCVIFarms~ state*newvalue+ newvalue+border_ind, data=finaldata2011, family=quasipoisson(link="log"))  
drop.p5<-update(drop.mod5, family=poisson)
drop.mod6<-glm(NumCVIFarms~ state+ newvalue+border_ind, data=finaldata2011, family=quasipoisson(link="log"))  
drop.p6<-update(drop.mod6, family=poisson)
QAIC(drop.p6, chat=chat) #


drop.mod7<-glm(NumCVIFarms~ state*newvalue, data=finaldata2011, family=quasipoisson(link="log"))  
drop.p7<-update(drop.mod7, family=poisson)
QAIC(drop.p7, chat=chat) # not selected, keep mod5 and drop main effects


summary(drop.mod5)
finaldata2011$state2<-relevel(finaldata2011$state, "iowa")
finaldata2011$state3<-relevel(finaldata2011$state, "texas")
finaldata2011$state4<-relevel(finaldata2011$state, "wisconsin")
finaldata2011$state5<-relevel(finaldata2011$state, "new york")
finaldata2011$state6<-relevel(finaldata2011$state, "minnesota")
finaldata2011$state7<-relevel(finaldata2011$state, "north carolina")
finaldata2011$state8<-relevel(finaldata2011$state, "nebraska")

summary(drop.mod5)
test.mod<-glm(NumCVIFarms~ state2*newvalue+ newvalue+border_ind, data=finaldata2011, family=quasipoisson(link="log"))  
summary(test.mod)
test.mod<-glm(NumCVIFarms~ state3*newvalue+ newvalue+border_ind, data=finaldata2011, family=quasipoisson(link="log"))  
summary(test.mod)
test.mod<-glm(NumCVIFarms~ state4*newvalue+ newvalue+border_ind, data=finaldata2011, family=quasipoisson(link="log"))  
summary(test.mod)
test.mod<-glm(NumCVIFarms~ state6*newvalue+ newvalue+border_ind, data=finaldata2011, family=quasipoisson(link="log"))  
summary(test.mod)
test.mod<-glm(NumCVIFarms~ state7*newvalue+ newvalue+border_ind, data=finaldata2011, family=quasipoisson(link="log"))  
summary(test.mod)
#NY
test.mod<-glm(NumCVIFarms~ state5*newvalue+ newvalue+border_ind, data=finaldata2011, family=quasipoisson(link="log"))  
summary(test.mod)
#NE
test.mod<-glm(NumCVIFarms~ state8*newvalue+ newvalue+border_ind, data=finaldata2011, family=quasipoisson(link="log"))  
summary(test.mod)
